...
cd /storage/home/users/pjt6/newton/final_genes/funaannot/update_results/subventral_gene_finding/
...
do
    cmd="python 
    get_sequences_i_want_from_fasta_command_line.py 
    Gp.all_${v}_upstream.fasta subventralcandidates.txt Gp_SUBVENTRAL_${v}_upstream.fasta"
    #echo ${cmd}
    #eval ${cmd}
done
...
for v in ${values}:
do
    cmd="findMotifs.pl Gp_SUBVENTRAL_${v}_upstream.fasta fasta BLAST_hit_candiates_sig_p_upJ2_homer_results_${v}/ -fasta GP_non_effectors_${v}_upstream.fasta"
    echo ${cmd}
    eval ${cmd}
done
